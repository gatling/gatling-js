...
public class JsFileSystem implements FileSystem {
  ...
  private final Map<String, List<String>> replacementsByResolutionPath;

  public JsFileSystem(FileSystem delegate, Map<String, List<String>> replacementsByResolutionPath) {
    this.delegate = delegate;
    this.replacementsByResolutionPath = replacementsByResolutionPath;
  }

  // FIXME cache?
  // FIXME match entire path?
  private String getReplacement(Path path) {
    for (Map.Entry<String, List<String>> entry : replacementsByResolutionPath.entrySet()) {
      String resolutionPath = entry.getKey() + "/";
      for (String replacement : entry.getValue()) {
        if (path.endsWith(replacement + ".js")) {
          return resolutionPath + replacement + ".js";
        }
      }
    }
    return null;
  ...
  public SeekableByteChannel newByteChannel(
    ...
    if (replacement != null) {
      ...
      try (var is =
        ...
        if (is == null) {
          System.out.println("Resource not found: " + replacement);
        }
        // FIXME null?
