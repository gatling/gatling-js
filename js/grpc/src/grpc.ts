...
import { GrpcDsl$MessageResponseTimePolicy as GrpcDslMessageResponseTimePolicyStatic } from "@gatling.io/jvm-types";
...
export const underlyingSessionToCredentials =
  ...
  (jvmSession: JvmSession) =>
    toJvmCredentials(f(wrapSession(jvmSession)));

export type MessageResponseTimePolicy = "FROM_STREAM_START" | "FROM_LAST_MESSAGE_SENT" | "FROM_LAST_MESSAGE_RECEIVED";

export const toJvmMessageResponseTimePolicy = (
  policy: MessageResponseTimePolicy
): JvmGrpcDslMessageResponseTimePolicy => {
  if (policy === "FROM_STREAM_START") {
    return GrpcDslMessageResponseTimePolicyStatic.FromStreamStart;
  } else if (policy === "FROM_LAST_MESSAGE_SENT") {
    return GrpcDslMessageResponseTimePolicyStatic.FromLastMessageSent;
  } else if (policy === "FROM_LAST_MESSAGE_RECEIVED") {
    return GrpcDslMessageResponseTimePolicyStatic.FromLastMessageReceived;
  }
